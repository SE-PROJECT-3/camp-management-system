...
@section('title', 'Edit Camp - ' . $camp->name)
...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-bold tracking-tight">Edit Camp Details</h2>
        <p class="text-slate-500 dark:text-slate-400 mt-2">Update the information for <span class="text-indigo-600 font-semibold">{{ $camp->name }}</span>.</p>
    </div>

    <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-8 shadow-sm">
        <form action="{{ route('camps.update', $camp) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" id="name" name="name" required value="{{ $camp->name }}"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="location" name="location" required value="{{ $camp->location }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                </div>

                <div class="space-y-2">
                    <label for="capacity" class="text-sm font-semibold tracking-wide px-1">General Capacity</label>
                    <input type="number" id="capacity" name="capacity" value="{{ $camp->capacity }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                </div>

                <div class="space-y-2">
                    <label for="families_count" class="text-sm font-semibold tracking-wide px-1">Families Count</label>
                    <input type="number" id="families_count" name="families_count" value="{{ $camp->families_count }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                </div>
            </div>
