...
@section('title', 'Edit Family - ' . $family->name)
...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-bold tracking-tight">Edit Family Details</h2>
        <p class="text-slate-500 dark:text-slate-400 mt-2">Update information for family: <span class="text-indigo-600 font-semibold">{{ $family->name }}</span>.</p>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-8 shadow-sm">
        <form action="{{ route('families.update', $family) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="name" name="name" required value="{{ $family->name }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="number" id="members_count" name="members_count" required value="{{ $family->members_count }}"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select id="category" name="category" required
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                        <option value="Displaced" {{ $family->category == 'Displaced' ? 'selected' : '' }}>Displaced</option>
                        <option value="Local" {{ $family->category == 'Local' ? 'selected' : '' }}>Local</option>
                        <option value="Refugee" {{ $family->category == 'Refugee' ? 'selected' : '' }}>Refugee</option>
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select id="priority" name="priority" required
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                        <option value="normal" {{ $family->priority == 'normal' ? 'selected' : '' }}>Normal</option>
                        <option value="high" {{ $family->priority == 'high' ? 'selected' : '' }}>High (Urgent)</option>
                        <option value="low" {{ $family->priority == 'low' ? 'selected' : '' }}>Low</option>
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select id="camp_id" name="camp_id" required
                        ...
                        @foreach($camps as $camp)
                            <option value="{{ $camp->id }}" {{ $family->camp_id == $camp->id ? 'selected' : '' }}>{{ $camp->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
